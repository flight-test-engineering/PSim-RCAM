...
def calculate_ground_forces(X:np.ndarray, h_cg:float) -> np.ndarray:
    ...
    for i in range(3):
        ...
        # Only calculate if gear is touching ground
        if delta_z > 0:
            
            # 1. Position of this specific gear relative to CG
            r_gear = gears_pos[i]
            
            # 2. Velocity of this gear tip (V_gear = V_cg + w x r)
            # This is crucial for the Damping term
            V_gear = V_cg_b + np.cross(wbe_b, r_gear)
            
            # 3. Normal Force (Vertical Spring-Damper)
            # F = K * displacement - D * velocity_z
            # We assume positive Fz is DOWN. Spring pushes UP (negative Z).
            # Velocity: Positive Vz is moving DOWN. Damping opposes motion.
            
            # Note: We use max(0, ...) for the spring term to ensure it never pulls the plane down
            F_normal = -max(0.0, LG_SPRING_K * delta_z) - (LG_DAMP_D * V_gear[2])
            
            # Check: Don't allow the ground to pull the plane down (adhesive force) 
            # if the damper overcomes the spring
            if F_normal > 0: F_normal = 0 
            
            # 4. Friction (Simplified)
            ...
            # 5. Moment generated by this gear
            # M = r x F
